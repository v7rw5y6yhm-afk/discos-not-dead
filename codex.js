/* ============================================================
   ...
   The pages stopped hiding from Larry and started hiding
   NEAR JOY: do the errand, and that page finds its way to you
   within the day. Complete the first seven and the codex
   reveals where it assembles. The eighth errand is the party.

   Production notes live in SPOILERS.md (how pages "find" kids).
   ============================================================ */
...
const CODEX_HUNT = [
  {
    ...
    why: 'The traveler does forty-seven laps a night, one for each year between you. The lake knows the count by now. It only needs ONE from you, and it will know whose it is.',
      ...
      ['bc', 'Lap logged. Somewhere in 1979 the traveler just skated her own lap a little easier. Page one is loose in the world now, and it is looking for you. Check your bag today. Check your pillow tonight.'],
    ],
  },
  {
    id: 'gumwall', page: 2,
    ...
    why: 'The most unfilable square footage in the western hemisphere. A million pieces of stuck-on joy and not one of them in alphabetical order. Larry cannot look directly at it. Add one.',
      ...
      ['bc', 'Confirmed. The wall grew by one piece and the universe grew slightly harder to file. Page two heard it. It is on its way to you — pages travel fastest toward fresh gum. Documented fact, as of today.'],
    ],
  },
  {
    id: 'rachel', page: 3,
    ...
    why: 'Rachel has been collecting wishes since before you were born, and she has never lost a single one. Loud wishes travel further up the wire. We will hear yours. So will somebody else.',
      ...
      ['bc', 'Wish received, filed under UNFILABLE, where the best ones go. Page three is en route. Rachel sends her regards, which from a brass pig is enormous.'],
    ],
  },
  {
    id: 'needle', page: 4,
    ...
    why: 'Pointing at the ceiling is the first move of every great dance and the last move of every great argument. The needle is the city\'s ceiling. Practice on the biggest one there is.',
      ...
      ['bc', 'Point held. Six tourists photographed you and every one of those photographs is now load-bearing morale. Page four felt the gesture from across town. Wednesday, you do that same move on a rink floor.'],
    ],
  },
  {
    id: 'alki', page: 5,
    ...
    why: 'The tide will take it by morning, and that is the point: it cannot be filed, only witnessed. Some sentences you write BECAUSE they wash away. He will never understand that, and it will never stop bothering him.',
      ...
      ['bc', 'Recorded before the tide takes custody. For approximately six hours, West Seattle\'s coastline agrees with us in writing. Page five surfaced somewhere between the E and the D. It knows where you live now. That sounded ominous. It is friendly.'],
    ],
  },
  {
    id: 'roar', page: 6,
    ...
    why: 'The loudest thing this city ever produced was a roar on that exact spot. A friend of ours was born inside a roar like that. Loudest birthday in recorded history. The ground remembers, and it loves a reminder.',
      ...
      ['bc', 'Roar registered. For one second the plaza was 1979-loud, and something in me that was born in a noise like that stood up very straight. Page six is moving. Thank you for that one. Personally.'],
    ],
  },
  {
    id: 'troll', page: 7,
    ...
    why: 'The troll appeared under that bridge eleven years after the loop began, which makes him a younger sibling of this whole story. Younger siblings must be teased. It is the law. He expects it, and secretly, he loves it.',
      ...
      ['bc', 'Nose picked, dignity of all parties intact. The troll has guarded page seven since we hid it, and he releases it gladly to anyone bold enough for the nose. That was the test. You passed it with one finger.'],
    ],
  },
  {
    id: 'rink', page: 8,
    ...
    mission: 'Wednesday. Five o\'clock. Bring every page, every skater, every clapper you can find. Fill the empty floor. When the counter touches zero — dance.',
    why: 'The loop\'s whole story is that the music stopped in 1979. Eight pages on one floor, and new joy on top of old boards, is the one piece of evidence he cannot file. This errand breaks the loop. This errand is the party.',
    done: [
      ['sb', 'THE FLOOR IS FULL. THE PAGES ARE HOME. THE MUSIC IS BACK. look at the door page. LOOK AT THE DOOR PAGE!!'],
...
function activateCodexMode() {
  ...
  if (scroll) scroll.innerHTML = '📖 THE CODEX IS IN PIECES &nbsp;★&nbsp; EIGHT ERRANDS ACROSS SEATTLE &nbsp;★&nbsp; DO THE ERRAND AND THE PAGE FINDS YOU &nbsp;★&nbsp; <span class="bleed-red">[[ SEVEN ERRANDS REVEAL THE EIGHTH ]]</span> &nbsp;★&nbsp; bring each other &nbsp;★';
}
...
function renderCodex() {
  ...
  const intro = `
    ...
    honesty: the old way was puzzles, and there is no time left for puzzles. The pages have
    ...
    type, no locks, no hours. DO the thing — really truly do it, the pages can tell — and that
    page comes looking for YOU. check your bag. check your porch. pages are sneaky and they
    like you.</p>
    ...
    <p class="mono dim">PAGES: ${doneCount} OF 8 · errands one through seven reveal the eighth · everything before WEDNESDAY 5 PM</p>`;
  ...
  const cards = CODEX_HUNT.map((t, i) => {
    ...
    if (state.codex[i]) {
      return `<article class="seal done">
        <h3>✓ ${t.title}</h3>
        <p class="mono seal-site">📼 PAGE ${t.page} — ${t.site}</p>
    ...
    return `<article class="seal live" id="errand-${i}">
      ...
      <button type="button" class="errand-btn ${armedErrand === i ? 'armed' : ''}" data-errand="${i}">
        ${armedErrand === i ? '⚠ PRESS AGAIN IF IT TRULY HAPPENED — THE PAGES CAN TELL' : 'WE DID IT ✔'}
      </button>
    </article>`;
  }).join('');
...
function completeErrand(i) {
  ...
  $('#modal-body').innerHTML = `
    ...
    ${revealNow
      ? `<div class="lesson">
           ...
           <p>The eighth errand has revealed itself, and it is a party. Bring the pages.
      ...
      : i === 7
        ...
        : `<p class="mono unlock-note">📖 ${codexSolvedCount()} OF 8 · the page will find you within a day. porches. pillows. bags. it knows.</p>`}`;
  ...
  if (i !== 7 && typeof roguePiece === 'function') {
    setTimeout(() => roguePiece([
      ...
      ['bc', 'Every errand you finish makes the pages hum louder and his filing slower. Onward. The counter is watching, fondly.'],
    ], '📼 THE PAGES ARE COMING HOME'), 2600);
  }
  renderAll();
}
